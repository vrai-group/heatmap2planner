...
class PointsOfInterestGenerator:
    def __init__(self, args):
        ...
        self.path =  './heatmaps/' + str(args["folder"]) + '/'
        ...
        for i in range(0,len(self.forbidden_areas)):
            Utils.draw_forbidden(check_img,self.forbidden_areas[i],(0,0,255),2)

        for i in range(0,len(self.rep_areas)):
            Utils.draw_shelf(check_img,self.rep_areas[i],(255,51,153),2)

        for i in range(0,len(self.shelves)):
            Utils.draw_shelf(check_img,self.shelves[i],(0,255,0),2)

        for i in range(0,len(self.capture_pois)):
            Utils.draw_point(check_img,(self.capture_pois[i].x,self.capture_pois[i].y),(255,255,0),2)
            
        Utils.show_img_and_wait_key('Shelves', check_img)
        #Utils.save_image('./heatmaps/store28/shelves.png', check_img)
    ...
    def run(self):
        ...
        for i in range(0,len(self.final_pois)):
            Utils.draw_point(img,(self.final_pois[i].x,self.final_pois[i].y),(0,0,255),2)
        for i in range(0,len(self.capture_pois)):
            Utils.draw_point(img,(self.capture_pois[i].x,self.capture_pois[i].y),(255,0,0),2)
        Utils.show_img_and_wait_key('Final PoIs', img)
        ...
        Utils.show_img_and_wait_key('Final Image', img,final_img)
        ...
        Utils.show_img_and_wait_key('Navigation Image', img,final_img)
        # Utils.save_image('./results/store28/final_img.png', final_img)

        ## OPTIMIZATION (Traveling Salesman Problem) SETUP
        tsp = Tsp(self.heatmap, self.final_pois, self.capture_pois, self.shelves, self.base_path + 'results/' + self.folder)
                    ...
                    if block_size >= 8:#and block_size <= 128:
                        for coord in coords:
                            ...
                            if Utils.get_patch_average(patch) > 200:# and Utils.get_patch_variance(patch) < 100:
                                ...
                                Utils.draw_roi(blocks_img,roi,(0,0,255))
                                Utils.draw_point(blocks_img,(int(x+block_size/2),int(y+block_size/2)),(0,255,0),2)
                            else:
                                Utils.draw_roi(blocks_img,roi,(255,0,0),1)

        Utils.show_img_and_wait_key('QuadTree Decomp',blocks_img)
        Utils.save_image('./heatmaps/store28/quadtree.png', blocks_img)
        return pois
    ...
    def calc_capture_pois(self):
        capture_pois = list()
        pois_d_min = self.calc_px_hfov(self.d_min) -3 #to have 10 cm overlap
        count = 0
    ...
    def calc_final_pois(self):
        pois_d_min = self.calc_px_hfov(self.d_min)  -3 #to have 10 cm overlap
        final_pois = self.capture_pois.copy()
        ...
        Utils.show_img_and_wait_key('Final POIS', img2)
        #Utils.save_image('./results/store28/final_pois.png',img2)
