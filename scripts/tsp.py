...
class Tsp:
  def __init__(self,img,pois,capture_pois, shelves, path, debug_mode=False):
    ...
    if self.debug_mode == True:
      ...
      P = np.add(np.add(D_norm,O),C_norm)
      for i in range(0,len(P)):
        for j in range(0,len(P[i])):
          if (P[i][j] > 9999999999):
            P[i][j] = 9999999999
      sequence = run_tsp(P,img,pois)
      self.draw_solution(img,sequence,pois,capture_pois)

    self.index_sequence = sequence

    self.final_path = self.order_pois()

  def draw_solution(self,img,sequence,pois,capture_pois):
   
    pt1 = (pois[sequence[0]].x, pois[sequence[0]].y)
    pt2 = (pois[sequence[1]].x, pois[sequence[1]].y)
    Utils.draw_arrow(img,pt1,pt2,(0,0,255),2)
    
    for i in range(1,len(sequence)-1):
      pt1 = (pois[sequence[i]].x, pois[sequence[i]].y)
      pt2 = (pois[sequence[i+1]].x, pois[sequence[i+1]].y)
      Utils.draw_arrow(img,pt1,pt2,(0,255,0),2)

    for poi in pois:
      Utils.draw_PoI(img,poi)
    
    for poi in capture_pois:
      Utils.draw_PoI(img,poi,(255,255,0))
  ...
  def calc_distance_matrix(self, pois):
    k1 = 1
    D = np.zeros((len(pois),len(pois)),dtype=np.int64)
    D_max = 0
    D_min = 9999999999
  ...
  def normalize_distance_matrix(self,D,D_max,D_min):
    D_norm = np.zeros((len(D),len(D)),dtype=np.int64)
    ...
    for i in range(0,len(D_norm)):
      for j in range(i+1,len(D_norm)):
        if D[i,j] != 9999999999 and D[i,j] != 0 :
          D_norm[i,j] =  np.int64((((D[i,j] - D_min) * new_range) / old_range) + self.min_range)
  ...
  def normalize_heatmap_matrix(self,C,C_max,C_min):
    ...
    C_norm = np.zeros((len(C),len(C)),dtype=np.int64)
    ...
    for i in range(0,len(C_norm)):
      for j in range(i+1,len(C_norm)):
        C_norm[i,j] =  np.int64((((C[i,j] - C_min) * new_range) / old_range) + self.min_range)
  ...
  def calc_orientation_weights(self,pois):
    k2 = 1
    O = np.zeros((len(pois),len(pois)),dtype=np.int64)
    ratio = np.int64((self.max_range - self.min_range) / 3)
      ...
      for j in range(i+1,len(pois)):
        if i != j:
          if pois[i].theta == -1 or pois[j].theta == -1:
            ...
            theta = np.int64(abs(pois[i].theta - pois[j].theta)) #we can have only 3 values: 90, 180, 270
            
            # print('(i,j) theta and difference: ',i,j, pois[i].theta, pois[j].theta,pois[i].theta - pois[j].theta )
            if theta == 90:     
              O[i,j] = np.int64(k2*ratio)
              O[j,i] = O[i,j]
            if theta == 180:     
              O[i,j] = np.int64(2*k2*ratio)
              O[j,i] = O[i,j]
            if theta == 270:     
              O[i,j] = np.int64(k2*ratio)
              O[j,i] = O[i,j]
